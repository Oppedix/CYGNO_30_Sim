#!/usr/bin/env python3
"""Resumable, isolated Table 7.1 smoke jobs. No production/chain certification."""
import argparse
...
LIMITATIONS = ['Pipeline smoke only; no scientific rate agreement or production authorization.',
               'Long-lived primary/daughter transport unvalidated; threshold is recorded, never changed.',
...
def load_config(path):
    ...
    require(set(config) == {'schema_version', 'mode', 'layout', 'model', 'source_policy',
                            'primaries_per_job', 'seed', 'timeout_seconds'}, 'Unknown/missing config field')
    require(config['schema_version'] == 1 and config['mode'] == 'smoke', 'Only smoke mode is implemented')
    ...
    require(type(config['primaries_per_job']) is int and 1 <= config['primaries_per_job'] <= 1000,
            'Smoke count must be 1..1000 primaries per contribution')
    require(type(config['seed']) is int and 1 <= config['seed'] <= 2_000_000_000, 'Invalid seed')
    require(type(config['timeout_seconds']) is int and 1 <= config['timeout_seconds'] <= 3600,
            'Smoke timeout must be 1..3600 seconds')
    return config
...
def invoke(command, cwd, label, timeout):
    """Durable command receipt even on nonzero exit or timeout. No shell."""
    ...
    try:
        with (cwd/(label+'.log')).open('w') as log:
            result = subprocess.run(record['command'], cwd=cwd, stdout=log, stderr=subprocess.STDOUT,
                                    timeout=timeout, check=False)
        record['returncode'] = result.returncode
        require(result.returncode == 0, f'{label} exited {result.returncode}; see {cwd/(label+".log")}')
...
def normalization(rows, jobs, quantities, identity):
    lines = ['# normalization-format: quantity-v2', f"# layout: {identity['layout']}",
             ...
             '# provenance: Table 7.1; actual construction and generated counts; PIPELINE SMOKE ONLY; chain validity UNVALIDATED']
    for row in rows:
...
def run(args):
    ...
    output = args.output.resolve(); build = args.build.resolve()
    ...
    with locked(output):
        ...
        (probe/'environment.mac').write_text('/control/echo CYGNO_ENVIRONMENT_PROBE\n')
        ...
        if campaign_path.exists():
            ...
            campaign = dict(schema_version=1, fingerprint=fingerprint, identity=identity,
                            ...
                            created_unix=time.time(), scientific_validity='unvalidated')
        ...
        for row in matrix['contributions']:
            if row['id'] in selected:
                try:
                    jobs[row['id']] = run_job(output, row, config, campaign, tools, expected, args.retry_failed)
                except (ValueError, RuntimeError, OSError) as error:
        ...
        report = dict(campaign_fingerprint=fingerprint, mode='pipeline-smoke', scientific_validity='unvalidated',
