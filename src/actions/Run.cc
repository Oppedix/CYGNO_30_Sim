...
void Run::Merge(const G4Run* run)
{
  ...
  //
  if (localRun->GetNumberOfEvent() > 0) {
...
void Run::EndOfRun() 
{
 ...
 G4String partName = fParticle ? fParticle->GetParticleName() : "no primary";
 ...
 G4cout << "\n Visible-secondary event energy is not accumulated in this configuration.\n";

 //activity of primary ion
 //
 if (fParticle && fParticle->GetAtomicMass() > 0 && fParticle->GetPDGLifeTime() > 0 && fPrimaryTime > 0) {
 ...
 G4cout << "\n   Activity estimate from sampled primary times for " << partName << " = "
